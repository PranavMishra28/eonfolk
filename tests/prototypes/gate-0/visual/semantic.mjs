...
export const SEMANTIC_ACTIONS = Object.freeze([
  Object.freeze({
    ...
    role: "button",
    name: "Follow Mara",
    description: "Follow Mara. She acts for herself; this does not command her movement or work.",
    enabled: true,
    focusable: true,
    keys: Object.freeze(["Enter", " "]),
  }),
  Object.freeze({ id: "people", role: "button", name: "People", description: "Open the named citizen list.", enabled: true, focusable: true, keys: Object.freeze(["Enter", " "]) }),
]);

export function createSemanticTree() {
  return Object.freeze([
    Object.freeze({ order: 1, role: "region", name: "Riverhold world", text: "Riverhold settlement: bridge, well, market, mill, woodpile, eight citizens, and three visible activity families." }),
    Object.freeze({ order: 2, role: "heading", name: "Mara", text: `${MARA_PROJECTION.autonomy}. ${MARA_PROJECTION.standingPlan} ${MARA_PROJECTION.visibleReason}` }),
    Object.freeze({ order: 3, role: "status", name: "Authoritative interaction", text: FIXTURE.authoritativeInteraction.label }),
    Object.freeze({ order: 4, role: "note", name: "Mara and Toma relationship", text: `${FIXTURE.relationshipCue.label} ${FIXTURE.relationshipCue.status}.` }),
    Object.freeze({ order: 5, role: "region", name: "Chronicle beat: Exchange settled", text: FIXTURE.chronicleBeat.text }),
    ...CITIZENS.map((citizen, index) => Object.freeze({ order: 6 + index, role: "listitem", name: citizen.name, text: citizen.activity, citizenId: citizen.id })),
    ...SEMANTIC_ACTIONS.map((action, index) => Object.freeze({ order: 14 + index, ...action })),
  ]);
}

export function activateSemanticAction(actionId, key) {
  const action = SEMANTIC_ACTIONS.find(({ id }) => id === actionId);
  if (!action || !action.enabled || !action.focusable || !action.keys.includes(key)) {
    return Object.freeze({ activated: false, actionId });
  }
  return Object.freeze({ activated: true, actionId, selectedCitizenId: actionId === "follow-mara" ? "citizen:mara" : null });
}
...
export function mountSemanticObserver(root, { onFollowMara = () => {} } = {}) {
  ...
  summary.textContent = "Riverhold settlement: bridge, well, market, mill, woodpile, eight citizens, and three visible activity families.";
  root.append(summary);
  ...
  note.textContent = "She acts for herself; following does not command movement or work.";
  const people = document.createElement("button");
  people.type = "button";
  people.textContent = "People";
  root.append(follow, note, people);
  return Object.freeze({ followButton: follow, peopleButton: people, citizenList: list });
}
