...
test("fixed gate0-visual-v1 fixture is deterministic and answer keys are exact", () => {
  ...
  assert.deepEqual(FIXTURE.authoritativeInteraction.actorIds, ["citizen:iven", "citizen:toma"]);
  assert.deepEqual(FIXTURE.relationshipCue.actorIds, ["citizen:mara", "citizen:toma"]);
...
test("Riverhold dominates all three required viewports and projection remains reproducible", () => {
  for (const viewport of Object.values(VIEWPORTS)) {
    ...
    assert.equal(createPixiCommands(first).filter(({ kind }) => kind === "citizen-sprite").length, 8);
  }
...
test("weak devices keep the same facts through semantic markers and discrete motion", () => {
  const projection = createProjection({ ...VIEWPORTS.mobile, weakDevice: true });
  assert.equal(projection.quality, "semantic-markers");
  assert.equal(projection.motion, "discrete");
  assert.equal(createPixiCommands(projection).filter(({ kind }) => kind === "marker").length, 8);
  assert.deepEqual(projection.citizens.map(({ id, activityId }) => [id, activityId]), CITIZENS.map(({ id, activityId }) => [id, activityId]));
});

test("semantic actions expose stable roles, names, order, and keyboard activation", () => {
  const tree = createSemanticTree();
  assert.deepEqual(tree.map(({ order }) => order), [...tree.keys()].map((index) => index + 1));
  ...
  assert.deepEqual(SEMANTIC_ACTIONS.map(({ role, name }) => [role, name]), [["button", "Follow Mara"], ["button", "People"]]);
  assert.equal(activateSemanticAction("follow-mara", "Enter").selectedCitizenId, "citizen:mara");
  assert.equal(activateSemanticAction("follow-mara", " ").activated, true);
  assert.equal(activateSemanticAction("follow-mara", "ArrowRight").activated, false);
});
...
test("ready predicate starts only after Pixi and equivalent semantic facts are complete", () => {
  ...
  assert.equal(isObserverReady({ ...facts, paintedCitizenIds: facts.paintedCitizenIds.slice(1) }), false);
  assert.equal(isObserverReady({ ...facts, semanticRowCitizenIds: facts.semanticRowCitizenIds.slice(1) }), false);
  assert.equal(isObserverReady({ ...facts, followMara: { ...facts.followMara, focusable: false } }), false);
});
...
test("observer prompt occurs on first delivered frame from 60,000 through 61,000ms", () => {
  ...
  assert.equal(processObserverFrame(started, 60_050.499, facts).promptDue, false);
...
test("observer endpoint requires both timers, four opaque responses, and durable persistence", () => {
  ...
  assert.equal(isObserverEndpointDurable({ ...record, durablyPersisted: false }), false);
  assert.equal(isObserverEndpointDurable({ ...record, responses: { ...record.responses, autonomy: null } }), false);
  assert.equal(isObserverEndpointDurable({ ...record, observationPromptMs: 61_001 }), false);
});
...
test("observable-oracle inputs close route, semantic tree, response surface, timeline, and ready predicate", () => {
  ...
  assert.deepEqual(first.viewportIds, ["desktop-1728x1117", "laptop-1366x768", "mobile-390x844"]);
  assert.deepEqual(first.responseSurface.map(({ answerToken }) => answerToken), Object.values(ANSWER_KEY));
...
test("responsive CSS names all required layouts, reduced motion, focus, targets, and semantic fallback", async () => {
  const css = await readFile(new URL("./gate0-visual.css", import.meta.url), "utf8");
...
test("participant-visible model contains no private codename, treatment IDs, or research framing", () => {
  const visible = JSON.stringify({ fixture: FIXTURE, tree: createSemanticTree(), pixi: createPixiCommands(createProjection(VIEWPORTS.desktop)) });
  assert.doesNotMatch(visible, /EONFOLK|\bFAM\b|\bTRI\b|\bFAC\b|\bECH\b|\bDIR\b|research|experiment/i);
});
