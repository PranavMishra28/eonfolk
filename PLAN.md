...
## World-as-Product release evidence — 2026-08-22

The final implementation head `f818d1069401a1f8e14d2ca6badec29841afbd81` passed protected GitHub Actions [run 32559456985](https://github.com/PranavMishra28/eonfolk/actions/runs/32559456985): `Verify` in 4m18s, `Formal model` in 20s, and full-history `Secret scan` in 11s. The Linux PR lattice recorded nineteen passing constituents: two semantic injected-fault journeys, fourteen semantic product journeys, and a blocking trace-free production-renderer smoke at 1728×1117, 1366×768, and 390×844. The smoke proved PlayCanvas/WebGL2 readiness, exact canvas/host sizing, pixel ratio 1, eight projected and semantic citizens, active visible work, desktop keyboard camera motion, zero browser errors, and zero external attempts. Accepted UI and verification-manifest artifacts use the declared 30-day retention; the PR output SHA-256 is `4e65374b3b314e7f7e4e22888228c61859171387671082b2759589cdf5f12da2` and artifact-manifest SHA-256 is `a6d87c4ce981d79ca6dfb27d750e6e89b5e3bf45d9cd58f3cb2c8fbfc2cd5c6e`.

The same exact clean implementation head then passed the complete target-Mac `verify:deep` lattice while plugged into AC power. All **26/26** ordered constituents passed with an unchanged source and lockfile, including **191 tests in 23 files**, the 500-run/320-command property profile, 8/8 targeted mutants, real IndexedDB, timing, two fault journeys, all sixteen illustrated production journeys, zero-egress checks, production audit, bounded TLC (3,480 generated/350 distinct states, depth 10, five invariants), browser-cohort validation, persistence measurement, OFF/LOCAL/ALPHA diagnostics, and the canonical fifteen-run performance battery. Verification output SHA-256 is `21e85a3e875ca79d0d6e31b25d4ba19f1c871bb6c22c780cf777ba877cfb689d`; artifact-manifest SHA-256 is `cc54184974c05e69e9be0c18d98ebd5d07769bac55bf96fcc1b650bf68dedddd`.

The canonical battery ran five fresh browsers for desktop, laptop, and throttled 390×844 mobile emulation. Pooled p95 frame time was **9.1 ms desktop**, **8.9 ms laptop**, and **9.2 ms mobile**; maximum meaningful-world display was **445.0 ms desktop**, **455.4 ms laptop**, and **4,433.4 ms mobile**. One isolated laptop arrival frame was 50.1 ms and counted as one dropped frame; the declared aggregate/p95 gate still passed without changing a limit. All fifteen arrival marks retained the required eight-citizen activity and interaction evidence. The 75 routed requests and fifteen independent netlogs contained zero external attempts. Payload was **99,896 bytes gzip** critical shell, **644,464 bytes gzip** total JavaScript, and **0 bytes** world assets. Canonical performance artifact SHA-256 is `d1a8fb491d58bd1dedd2cbfade3af08b7191220f3930afcc50e4c2bb1446278e`.
...
| Shared authority synthesis | `7a1ba77`, `d9f7c20`, plus later candidate updates | Historical checkpoint: every frozen P0/P1 and all seven first-confirmation P1s received an explicit disposition and direct repair evidence; later rows record the completed confirmation and clean lattice. |
...
| World-as-Product implementation | `53c1a2d`–`3477f79` | Historical implementation checkpoint: integrated a 250×210 m PlayCanvas world, one-metre scale manifest, authored cells/places/affordances, Reality-owned reservations and semantic travel, no-premature-arrival projection, three semantic camera scales/LOD, direct canvas selection and mobile sheet parity, Chronicle spatial targets, an inhabited opening composition, and focused browser/unit truth gates. Later review/repair/confirmation rows supersede its then-pending evidence state. |
...
| Opening-lifetime repair and final confirmation | repair `4c205e2`; frozen candidate `90c0ad2`; confirmation `1da2c8a`, integrated as `40e2154` | Reality—not the renderer—keeps the market exchange reserved through simulation time 180 and settles it once at 240. The unchanged throttled predicate qualified at 4,439.5 ms with a current illustrated interaction; exact answer **YES**, zero P0/P1. |
| Historical pre-CI-repair PlayCanvas DEEP | coordinator at `e349c5c` | All 26 rows passed from clean unchanged source/lockfile, but later build-identity and Linux-CI repairs superseded this as the release head without invalidating its historical evidence. |
| Final implementation-head remote CI | coordinator at `f818d10`; run `32559456985` | Protected `Verify`, `Formal model`, and full-history `Secret scan` passed. Linux retained two semantic fault journeys, fourteen semantic product journeys, and a real trace-free three-viewport PlayCanvas/WebGL2 smoke with zero errors/egress. |
| Final implementation-head target-Mac DEEP | coordinator at exact clean `f818d10` | All 26 rows, 191 tests, sixteen illustrated journeys, 8/8 mutants, bounded TLC, diagnostics/persistence, and fifteen performance repetitions passed from unchanged source/lockfile. Output SHA-256 `21e85a3e875ca79d0d6e31b25d4ba19f1c871bb6c22c780cf777ba877cfb689d`. |
