...
**Status:** VERIFIED CURRENT-SESSION INVENTORY — 2026-08-23; reopen after an environment change
...
No optional external connector was connected or purchased. Cloudflare guidance arrived through the active Codex environment; it was not used to connect an account or mutate infrastructure. Browser, project Playwright, Image Generation, and authenticated GitHub capabilities were used only within authorized work. Read-only probes did not print tokens.

## Executive recommendation

Proceed with the active Browser capability, pinned project Playwright 1.62.1, built-in Image Generation, and authenticated GitHub CLI when their corresponding work is authorized. The pinned Playwright cohort now supplies the repeatable browser E2E and screenshot capability required by V1; absence of a global `playwright` binary is intentional.

Do not install Codex Game Studio, Build Web Apps, Promptfoo, shadcn MCP, Motion AI Kit/MCP, or 21st MCP for V1. Cloudflare's official guidance skills are now active, but no account connector, CLI, credential, or deployment path is active or required. Reconsider Cloudflare execution only at an explicitly authorized hosted-region gate and Promptfoo only if the existing deterministic cognition suite becomes insufficient. shadcn guidance is active; Motion and 21st remain default-reject optional accelerators because they add remote/code/provenance surfaces without V1 necessity.

## Current environment observations

**VERIFIED FACT (S-TOOL-01):** the 2026-08-23 active session lists:
...
- official Cloudflare platform, Durable Objects, Workers, Wrangler, security, and performance guidance skills;
- `vercel:shadcn` guidance;
- `sites:sites-building`, which is a Sites-specific builder and is not the named Build Web Apps plugin.

It does not list Codex Game Studio, Build Web Apps, Promptfoo, a shadcn MCP server, Motion tooling/MCP, or 21st tooling/MCP. Cloudflare guidance being active does not imply an account connection or deployment authority.

**VERIFIED FACT (S-TOOL-02):** read-only executable probes found `gh`, `node`, `pnpm`, and `npx`; did not find global `wrangler`, `promptfoo`, `playwright`, `agent-browser`, `shadcn`, or `motion-ai` commands. The repository-local `pnpm exec playwright --version` returned 1.62.1. `gh auth status` confirmed one active GitHub login from the system keyring with `gist`, `read:org`, `read:packages`, `repo`, and `workflow` scopes. This is broad write authority, not evidence that every write is authorized.
...
| Browser / Playwright | **AVAILABLE:** in-app Browser plus pinned project Playwright 1.62.1; no global CLI | Active manifest and successful localhost/browser-suite runs [S-TOOL-01][S-TOOL-02]; Playwright docs [S-TOOL-05] | Exploratory interaction and screenshots plus reproducible E2E, traces, viewport evidence, and visual comparison | Browser control can access visible pages and possibly signed-in sessions; project Playwright launches the frozen browser and writes evidence artifacts | Active Browser cost not separately exposed; Playwright is OSS/local compute | Signed-in session exposure, unintended web actions, screenshot data, hostile pages, flaky visual baselines | **REQUIRED** |
...
| GitHub | **AVAILABLE and authenticated** through `gh` | Local probe [S-TOOL-02]; official CLI [S-TOOL-07] | Inspect repos/PRs/issues/CI; branch, commit, push, and review workflows when authorized | Current token scopes include broad repository and workflow authority; local git can write branches/commits | GitHub CLI is MIT/free; account plan, Actions, security features, and storage costs are separate | Accidental push/merge/comment, secret exposure in logs, broad token scope, untrusted Actions/PR code | **REQUIRED** |
| Cloudflare guidance/plugin | **AVAILABLE as official guidance skills; NOT CONNECTED for account operations; `wrangler` absent** | Active session manifest [S-TOOL-08]; Cloudflare docs for service terms [S-TOOL-09] | Architecture, Durable Objects, Workers, security, performance, and Wrangler guidance; no current account mutation capability | Current skills can guide local work. Any future CLI/connector/account credential or deployment would need separate explicit authorization | Skills add no runtime spend; Workers/DO/AI have Free and paid limits | Mistaking guidance for authority, later credential/account access, deployment/state mutation, logs/secrets, denial-of-wallet | **OPTIONAL until hosted-region gate** |
...
**VERIFIED FACT:** Browser-related skills are active; the in-app Browser opened, inspected, and captured the owned localhost application at all three target viewport classes. Global `playwright` and `agent-browser` executables remain absent, while repository-local `pnpm exec playwright --version` returns the pinned 1.62.1 cohort and the production suites use its installed Chrome at desktop, laptop, and mobile sizes [S-TOOL-02]. Playwright supports screenshot comparisons and warns that rendering varies with OS, browser, hardware, settings, and headless state [S-TOOL-05].

**INFERENCE:** use the active Browser for authorized exploratory/manual QA. During implementation, add `@playwright/test` **1.62.1** through the project lockfile rather than relying on a global command. Its official package metadata pins Chromium revision **1234**, Chrome for Testing **151.0.7922.34**; acceptance verifies the complete installed app-bundle manifest, not only its launcher [S-TOOL-17]. Generate visual baselines in that controlled browser cohort and still do real M4 Pro/browser/device inspection for canvas/WebGL states.
...
**VERIFIED FACT (S-TOOL-19):** the exact required headed Chromium 1234 / Chrome for Testing 151.0.7922.34 app is already present locally. Independent Node/Ruby no-follow enumeration matched 326 regular files, five symlinks, 372,002,382 regular-file bytes, a 62,239-byte manifest and SHA-256 `25995bc88bf20b6de47b46eb3571b250989846a797c0b8924b8794627b6175fc`; the 237,813,488-byte framework binary hashes to `269114cf695f1c50b54e0816a1442e41dc468d28672e2dedc2036105fb5a8dbe`. Goal mode must validate the complete bundle and stop rather than downloading on mismatch.

**VERIFIED FACT (S-TOOL-20):** after verified unused-dependency removal, the exact direct cohort resolves with pnpm 11.15.1 to 235 external packages; every retained lock integrity and complete license/lifecycle record is frozen in [DEPENDENCY_COHORT.md](DEPENDENCY_COHORT.md). CI validates those bytes rather than resolving a new transitive graph.
...
**VERIFIED FACT:** official Cloudflare guidance skills are active in this session, including platform, Durable Objects, Workers, Wrangler, security, and performance guidance. No `wrangler` executable, connected account tool, credential, or deployment authorization is present [S-TOOL-02][S-TOOL-08].

**INFERENCE:** use the active skills only as guidance during local V1 work. Do not install the CLI, connect an account, add credentials, or deploy until a hosted-region gate is separately authorized. At that gate, prefer a least-privileged token/account, preview/local test first, explicit resource names, a cost cap, and separate approval for deployment or destructive resource changes.
...
|---|---|---|---|---|---|---|
| S-TOOL-01 | Exact active skills/tools availability for this session, including Browser, Image Generation, shadcn guidance, and Cloudflare guidance skills | Codex session skills/tools manifest supplied to the agent | 2026-08-23 | A, environment | High for this session | Re-list every execution session |
| S-TOOL-02 | `gh` authenticated with stated scopes; repository Playwright is 1.62.1; global `wrangler`, `promptfoo`, `playwright`, `agent-browser`, `shadcn`, and `motion-ai` remain absent from PATH | Read-only local `command -v`, `pnpm exec playwright --version`, and redacted `gh auth status` probes | 2026-08-23 | A, local | High for this worktree/session | Re-probe after environment change |
...
| S-TOOL-08 | Official Cloudflare guidance skills are active, while no account connector, credential, deployment tool, or global Wrangler command is active | Codex session skills/tools manifest plus local executable probe | 2026-08-23 | A, environment | High for this session | Reinspect capabilities and permissions before any account connection or deployment |
...
| S-TOOL-19 | Local Playwright Chromium revision 1234 / Chrome for Testing 151.0.7922.34 complete app bundle matches the frozen no-follow manifest, framework hash, launcher path/version/hash and counts | Read-only local Node/Ruby lstat/hash enumeration, `--version`, and SHA-256 probes | 2026-08-20 | A, local | High for this machine/session | Reverify complete bundle at invocation/cohort/final oracle; stop instead of downloading on mismatch |
| S-TOOL-20 | pnpm 11.15.1 produced the reduced frozen 235-package graph; retained lock integrity and complete license/lifecycle records are captured in the cohort | [Official npm registry](https://registry.npmjs.org/); [frozen evidence](DEPENDENCY_COHORT.md) | 2026-08-23 | A, exact local + official registry | High for exact recorded bytes | Reopen only on approved cohort change, blocking build failure, or advisory |
