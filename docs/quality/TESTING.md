...
All producer lanes are manual dispatches in the already registered `.github/workflows/ci.yml`; no second workflow or protected-main bootstrap is required. Evidence uses three distinct commits: `initialReviewSha` is the source inspected by six reviewers, `frozenCandidateSha` is the reconciled software and immutable control, and `evidenceSha` adds only allowed evidence and structured review bytes. The frozen control manifest binds the exact commit plus Git blob IDs and mode `100644` for CI, the GitHub evidence verifier, readiness checker, 30-step DEEP runner, dependency validator, formal toolchain, and formal checker. Finalizers freshly check out frozen control and evidence, reject any changed control blob or mode before reading evidence bytes, and never execute evidence-checkout code.

The repository variable `V1_EVIDENCE_RUN_IDS_JSON` may enumerate exactly eight final run IDs under `eonfolk-v1-evidence-run-ids-v1`; it is an index, never a trust assertion. Ready CI derives the expected evidence commit from the exact PR head before merge and from the protected-main review envelope after merge, then queries GitHub and downloads artifacts into `RUNNER_TEMP`. It verifies successful manual runs, repository, actor, attempt, CI workflow identity/path, that expected evidence SHA, source and three-SHA ancestry, hosted finalizer identity, artifact name/archive digest/bytes, exact purpose roster, control blobs at frozen, evidence, and executed workflow-source commits, reviewer payload hashes, and seven unique reviewer agent/session pairs. The readiness checker reads review and confirmation bytes from `evidenceSha`, not the mutable checkout. Missing or expired runs, wrong metadata, duplicate identities, absent producers, changed controls, or byte/hash mismatches fail closed.
...
Target-Mac production is two-stage. First, an authorized administrator uses **System Settings → Users & Groups → Add User → Standard** to create a temporary `eonfolk-ci` account; the current interactive account is an administrator and noninteractive `sudo` is unavailable, so Codex cannot silently create this boundary. Log into that account, confirm `id -u` is nonzero and `dseditgroup -o checkmember -m "$(id -un)" admin` reports `no`, download the GitHub-hosted runner from the repository's **Settings → Actions → Runners → New self-hosted runner** page, and verify its published SHA-256 before extraction. Use the page's one-hour registration token only in the foreground command `./config.sh --url https://github.com/PranavMishra28/eonfolk --token <one-hour-token> --ephemeral --unattended --name eonfolk-deep-<nonce> --labels eonfolk-ephemeral-deep`, then run `./run.sh` as that standard user. Do not install a service or save the token.

The hosted preflight requires that unique runner online, idle, and carrying exact labels `self-hosted`, `macOS`, `ARM64`, and `eonfolk-ephemeral-deep`. If the workflow token cannot list repository runners, the user is admin/root, labels differ, or the runner is not deregistered after its job, the lane fails closed. After finalization, delete the extracted runner directory, log out, and have the administrator delete the temporary account and its home folder; a final `gh api repos/PranavMishra28/eonfolk/actions/runners` must contain no matching name. `sandbox-exec` is available but is not treated as a substitute for OS-user separation. No PR event targets a persistent self-hosted runner.

Dispatch `target-mac-intermediate` from `frozenCandidateSha`. A hosted preflight verifies runner availability; the Mac runs the exact 30-step DEEP under frozen control and uploads only a raw intermediate. After the ephemeral runner exits and deregisters, a clean GitHub-hosted finalizer verifies teardown and converts the raw bytes into a finalized intermediate. After reviews are reconciled and their JSON plus the final confirmation are committed as `evidenceSha`, dispatch the eight final purposes from that exact SHA. The hosted finalizers consume the finalized Mac intermediate or read structured review bytes only from `evidenceSha`; the run-ID registry then lets ready CI verify the complete cohort without self-referential run IDs in committed evidence.
...
After merge, do not clean the remote mega branch yet. Use a merge commit that preserves the mega-branch `evidenceSha` as an ancestor; squash or rebase merge cannot satisfy this post-merge protocol and must not be selected. Repeat the Mac intermediate and eight final dispatches from protected `main`, retaining the original immutable `evidenceSha` input while the workflow source becomes the merge commit. The verifier proves `evidenceSha` is an ancestor of that protected-main workflow source and that their control blob IDs/modes match. Replace the run-ID index with the post-merge cohort and rerun ready verification until every receipt is classified `POSTMERGE_PROTECTED_MAIN`. Only that re-attestation may support the protected-main release claim and remote branch cleanup. Dispatches are manual and capped; do not run them if they could consume paid minutes without approval.
