...
All producer lanes are manual dispatches in the already registered `.github/workflows/ci.yml`; no second workflow or protected-main bootstrap is required. Evidence uses three pairwise-distinct commits: `initialReviewSha` is the source inspected by six reviewers, `frozenCandidateSha` is the reconciled software and immutable control, and `evidenceSha` adds only allowed evidence and structured review bytes. The frozen control manifest binds the exact commit plus Git blob IDs and mode `100644` for CI, the GitHub evidence verifier, readiness checker, 30-step DEEP runner, dependency validator, formal toolchain, and formal checker. `workflowSourceSha` is separate: it is the commit targeted by an immutable temporary lightweight tag. Finalizers freshly check out frozen control, evidence, and workflow source, reject changed control blobs or modes before reading evidence bytes, and never execute evidence-checkout code.

The repository variable `V1_EVIDENCE_RUN_IDS_JSON` may enumerate exactly eight final run IDs under `eonfolk-v1-evidence-run-ids-v1`; it is an index, never a trust assertion. Ready CI derives the expected evidence commit from the exact PR head before merge and from the protected-main review envelope after merge, then queries GitHub and downloads artifacts into `RUNNER_TEMP`. It verifies the personal-repository owner and operator are `PranavMishra28`, merge commits are enabled while squash/rebase are disabled, `delete_branch_on_merge` remains `false`, the temporary tag still resolves to `run.head_sha`, successful manual runs, attempt-specific artifact names, CI workflow identity/path, source ancestry, hosted finalizer identity, artifact digest/bytes, the exact purpose roster, control blobs at frozen/evidence/workflow-source commits, reviewer hashes, and seven unique reviewer agent/session pairs. The workflow token uses only `actions: read` and `contents: read`; it never calls the administration-only repository-runners endpoint. A rerun leaves multiple attempt-specific evidence artifacts and therefore fails closed; use a fresh dispatch instead.
...
Target-Mac production is two-stage. First, an authorized administrator uses **System Settings → Users & Groups → Add User → Standard** to create a temporary `eonfolk-ci` account; the current interactive account is an administrator and noninteractive `sudo` is unavailable, so Codex cannot silently create this boundary. Log into that account, confirm `id -u` is nonzero and `dseditgroup -o checkmember -m "$(id -un)" admin` ends with `is NOT a member of group admin`, download the GitHub-hosted runner from the repository's **Settings → Actions → Runners → New self-hosted runner** page, and verify its published SHA-256 before extraction. Choose a unique 8–64 character nonce. Use the page's one-hour registration token only in `./config.sh --url https://github.com/PranavMishra28/eonfolk --token <one-hour-token> --ephemeral --unattended --name eonfolk-deep-<nonce> --labels eonfolk-ephemeral-deep-<nonce>`, then run `./run.sh` as that standard user. Do not install a service or save the token.

Before registration, the coordinator must run `gh api repos/PranavMishra28/eonfolk/actions/runners --jq '.total_count'` and observe zero. The first self-hosted step—before checkout or any action—requires the exact nonce-derived runner name, nonroot UID, a successful explicit `dseditgroup` command, and its exact nonmembership result. GitHub's actions-read jobs API later binds the successful job's positive runner ID, exact name, and exact sorted label set `ARM64`, `macOS`, `self-hosted`, and `eonfolk-ephemeral-deep-<nonce>`. No workflow step lists repository runners.

After the intermediate run, the coordinator repeats the external `gh` runner query and observes that the exact runner ID/name is absent, then commits `mac-runner-probe.json` at `evidenceSha`. Its closed schema records repository, intermediate run ID and attempt, runner ID/name/nonce, zero-runner before-registration observation, absent-after-run observation, timestamps, the owner/operator, boundary `COORDINATOR_RECORDED_GH_API_OBSERVATION_NOT_CRYPTOGRAPHIC`, and a SHA-256 over the JSON without `outputSha256`. This receipt is byte-bound and cross-checked with live jobs metadata, but the observations are procedural and not cryptographic. Delete the runner directory and temporary Standard account only after recording the second observation. `sandbox-exec` is not a substitute for OS-user separation.

Never dispatch by raw SHA. Create and push a lightweight `eonfolk-evidence-<nonce>` tag at `frozenCandidateSha`, then dispatch `target-mac-intermediate` with `gh workflow run ci.yml --ref eonfolk-evidence-<nonce> ...`; the hosted control preflight and later API verification require the tag target to equal `run.head_sha`. The Mac runs the exact 30-step DEEP and a clean hosted finalizer binds raw bytes to actions-read job metadata. After the external runner-probe receipt, reviews, reconciliation, and final confirmation are committed as `evidenceSha`, create a new lightweight evidence tag at that commit and dispatch the eight final purposes from it. Evidence concurrency is grouped by purpose with cancellation disabled; operationally dispatch each purpose sequentially and use fresh runs, not reruns. Retain both tags until the associated ready verification passes.
...
Live repository policy must retain `delete_branch_on_merge=false`, and the verifier rejects any drift before integration. Use a merge commit that preserves the mega-branch `evidenceSha` as an ancestor; squash or rebase cannot satisfy this protocol. Do not clean the branch or temporary tags after merge. Repeat the zero-runner probe, non-admin Mac intermediate, external absence probe, and eight final dispatches using fresh immutable tags at protected `main`, while retaining the original `evidenceSha` input. Replace the run-ID index and rerun the failed merge-push workflow until every receipt is `POSTMERGE_PROTECTED_MAIN`. Only then may the coordinator delete the mega branch and temporary tags. Dispatches are manual and capped; do not use paid minutes without approval.
