...
Release evidence is now produced only by the manually dispatched `V1 release evidence` workflow as protected-main control code. The repository variable `V1_EVIDENCE_RUN_IDS_JSON` may contain exactly eight unique run IDs under `eonfolk-v1-evidence-run-ids-v1`; it is an index, not a trust assertion. Ready CI executes the verifier and readiness checker extracted from the immutable PR-base `main`, queries GitHub live, downloads each artifact, and fails closed unless it verifies the repository, successful manual run, workflow path/name, protected-main workflow source, operator actor, run attempt, purpose, source/candidate relationship, producer job/runner labels, artifact identity, GitHub archive digest, payload bytes/hash, and exact eight-purpose roster. The temporary registry is created under `RUNNER_TEMP` and is never committed.

The API establishes that `main` is protected at verification time and that every workflow source SHA is in its current ancestry; GitHub does not expose proof that protection was continuously enabled at each historical dispatch. Nor can this personal-repository design cryptographically stop a same-repository candidate from rewriting its PR workflow shell: exact CI diff inspection, the frozen CI/security review, and the separate protected-main producer runs remain required procedural controls. A platform-enforced required workflow or external verification app would strengthen that boundary, but neither is assumed available or authorized here.

This protocol has a deliberate bootstrap boundary: `.github/workflows/v1-evidence.yml`, `scripts/v1-github-evidence.mjs`, and the compatible readiness/runner controls must first land on protected `main` through a narrowly reviewed control-plane bootstrap. The current mega PR cannot use its own untrusted copy to attest itself. Until that protected-main control exists, ready-mode verification must fail and `V1 READY` must not be claimed. Draft mode may run candidate tooling, but it always compares the current `GOAL.md` roster and immutable structure with the exact PR base and rejects a shrunken or rewritten roster.

For target-Mac DEEP, register a fresh GitHub Actions runner on the authorized M4 Pro with the official ephemeral registration option and exact labels `self-hosted`, `macOS`, `ARM64`, and `eonfolk-ephemeral-deep`. Dispatch `target-mac-deep` from `main` with one full candidate SHA and empty artifact/reviewer inputs. Protected control checks out that SHA without credentials, runs the exact 30-step DEEP roster, prepares the receipt, and retains it for 30 days. Remove the runner service, registration, working directory, and token after the single job. The API proves the selected labels and runner name; it does **not** prove the registration command used `--ephemeral`, so clean one-job runner disposal remains an operator-verifiable procedure, not a cryptographic claim. PR events never target this runner.

Run six fresh reviewer agents against the same frozen source and a seventh fresh confirmation agent after reconciliation. Each structured JSON artifact records a unique `reviewerAgentId` and `reviewerSessionId`. Manually dispatch the matching review/confirmation purpose on Ubuntu with the artifact path and those two values. Protected control treats the candidate checkout as inert bytes and never executes it. Hosted review receipts are capped at ten minutes and are neither scheduled nor automatic; do not dispatch them if the account has no included Actions allowance or the run could incur unapproved spend.

The GitHub actor is the human/operator account that dispatched the workflow. Reviewer agent/session fields are self-reported identifiers whose artifact binding and seven-way uniqueness are checked; they are not cryptographic identities and do not prove cognitive independence. A fresh-context reviewer process plus recorded task/session identity is the procedural trust root. Expired artifacts, absent runs, duplicate reviewer sessions, wrong actors/workflows/repositories/sources, or byte/hash mismatches require new producer runs rather than a configuration edit that asserts success.
