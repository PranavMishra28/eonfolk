...
The PR browser stage separates **two explicitly test-only crash-injection journeys** from **fifteen production acceptance journeys**. It runs and zero-egress-checks the fault suite first, rebuilds with the exact production configuration, measures that production bundle, then previews the existing `dist` without another build for all fifteen acceptance journeys. The verification wrapper fails closed if that final `dist` is empty or retains either crash-injection marker; its PR artifact manifest hashes only those inspected production files. DEEP manifests additionally admit only the four benchmark outputs produced by that tier. Stale DEEP files in `tmp` therefore cannot enter a PR manifest.
