...
Before each cohort, freeze and commit build/snapshot, a study seed derived from `PLAN_BASE` plus gate ID, assignment, exact script/questions/anchors/timers/scoring code, objective answer keys, and blank manifest; hash it. An instrument change voids and restarts that cohort. Cohorts are distinct, age 18+, able to read/use the prototype, unexposed to EONFOLK/Riverhold, and not contributors/reviewers. The operator obtains affirmative agreement after stating voluntary/unpaid participation, anonymous task data only, no name/contact/recording, and stop/skip rights. Enrolled withdrawal stays in the denominator; no replacement. Operator evidence contains raw opaque selections and ratings, not treatment-visible judgments; the analyzer scores exact fixture/receipt tokens before applying hidden row or condition mapping.
...
- 3/5 fresh context-free card viewers, after five seconds, answer `the player/you` as adviser, Mara's fixture choice, what followed, `advice contributed but did not directly act/change the rule`, and the unresolved tension. One branch/card is selected and committed before exposure by the Goal prompt's exact `cardSeed` rejection rule; all five view that card.
