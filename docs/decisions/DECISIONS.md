...
| FA-QA-001 | ACCEPT — CLOSED IN CODE; TARGETED CONFIRMATION PENDING | The first integrated DEEP run correctly failed closed after every preceding constituent passed because the canonical performance harness retained the same obsolete counsel label already removed from the diagnostics harness. The current repair aligns both scripts to `Review Mara's choices` and adds one source-contract regression covering both DEEP browser journeys. The failed run and partial benchmark are not accepted as performance evidence. |
