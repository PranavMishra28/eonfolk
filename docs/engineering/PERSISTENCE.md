...
The first slice keeps one local world. The loopback world-authority process stores a versioned authority stream in an atomic JSON file under `~/.eonfolk/worlds/`. The browser fallback stores the same `VersionedPersistencePort` shape in IndexedDB. Both adapters keep a Canonical World Ledger, a separate bounded Cognitive/Decision Ledger, and one immutable Experiment Manifest. One crash-safe genesis transaction creates the run. A transition becomes visible only after its batch header/events, durable head, command receipt, current fencing token, and associated consequential-decision record commit atomically. The V1 exact-version authority stream is implemented for both adapters with separate stream, operation, event, receipt, and snapshot stores; CAS heads, fencing, idempotency, corruption checks, exact migration, snapshot-plus-suffix replay, and deterministic catch-up are executable. The slice cannot yet back up, export, import, replace, fork, merge, or promote a world through a player-facing workflow; that honest limitation is disclosed before commitment.
...
The generated civilization has two local writers: the loopback world-authority process (file-backed, continues without a browser) and the generated civilization browser path (versioned IndexedDB). Real Chromium tests cover the IndexedDB path: day-365 persistence, close/reopen, exact replay, crash boundaries, corruption and stale writers. Unit tests cover file-backed genesis, restore, and honest process catch-up. Unit/property tests cover every reviewed catch-up horizon with zero model invocation. Migration remains deliberately narrow: one reviewed legacy checkpoint schema is fully revalidated into a new stream, and unknown or future versions fail closed. Cognitive decision-ledger composition and user-facing backup/export remain separate requirements.
