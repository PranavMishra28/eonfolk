...
POMCP/MCTS is not promoted for V1 under [D-016](../decisions/DECISIONS.md#d-016--do-not-promote-pomcp-without-a-calibrated-generative-model): the generated civilization has no calibrated stochastic transition/observation model. Search state may contain only `DecisionContext` fields and bounded planner-local bookkeeping; it cannot read canonical state directly or emit more authority than one `IntentProposal`.
