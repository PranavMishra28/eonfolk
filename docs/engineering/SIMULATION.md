...
- `contextHash`, `catalogHash`, `proposalHash`, and `decisionRecordHash` use the analogous domains `EONFOLK:DECISION-CONTEXT:v1`, `EONFOLK:ACTION-CATALOG:v1`, `EONFOLK:INTENT-PROPOSAL:v1`, and `EONFOLK:DECISION-RECORD:v2`, each over one JCS field containing the complete typed object without its own hash field. The exact proposal canonical bytes are stored beside `proposalHash`.
