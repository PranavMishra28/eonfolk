...
**Status:** RECONCILED; FIRST CONFIRMATION FAILED AT `1834f1f`; REPAIRED CANDIDATE HAS CANONICAL PERFORMANCE AND AWAITS FINAL TARGETED CONFIRMATION
...
| IFR-CI-004 | P1 | Performance was a one-run smoke rather than the planned repeated profile. | ACCEPT | The canonical headed protocol completed five clean cold contexts per profile, 5-second warmups plus 30-second arrival/busy-market/Chronicle samples, required desktop/laptop/mobile profiles, throttled mobile load, post-load offline completion, and dual zero-egress oracles. Every numerical gate passed at exact clean source `c44f05c`; the [canonical evidence](../exec-plans/evidence/001/implementation/performance.json) includes source, lockfile, power boundaries, per-run and pooled summaries. |
...
| IFR-CONF-003 | P1 | Five-run performance evidence still violated the canonical headed, cold-context, throttled, 30-second-state procedure. | ACCEPT | The new benchmark and wholly new clean-commit run implement the full procedure, one-shot next-frame marks, per-state/pooled distributions, latency/long-task/memory diagnostics, exact source/power metadata, and route/netlog oracles. Networking is disabled after meaningful-world display and every full journey completes; normal-mode battery remained 82% to 78% under DEV-M5-004 with unchanged numerical budgets. |
...
The repaired working tree passes complete `pnpm verify`: runtime/cohort/architecture/docs/format/lint/typecheck, 61 unit tests, two property tests, real Chromium IndexedDB reload, the 20/200-cycle timing analyzer, production build/bundle budgets, zero-advisory dependency audit, six Playwright journeys, dual zero-egress oracles, and bounded TLA+. Private-checkpoint Verify/Formal/Secret jobs are green at `54fca9e`; the canonical 15-run clean-commit performance lab passes every numerical, provenance, power, and egress gate at `c44f05c`. Final exact-commit CI/Gitleaks and targeted confirmation remain pending. This is automated/local evidence only.
