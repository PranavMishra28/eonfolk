...
- 2026-08-21 — The generated cognition smoke descriptor hash is `cb1713b932e1a848a264ac3fcf7788b42ce281a17306887ebf39e2beeb965596`. It exercises Standard Brain output determinism and legality, but does not freeze all exact context/catalog/oracle bytes or derive hidden equivalence, scenario goals, and terminal vectors through an authoritative runner. Planner disposition is `defer-no-candidate`; no Planner or model runtime ships.
- 2026-08-21 — Confirmation repair binds each experiment manifest to an ordered context/seed/repetition execution plan and accepts only in-order per-execution evidence. Completed-run, invocation, and success counts are derived by the journal; incomplete, duplicate, reordered, extraneous, mismatched, tampered, failed-invariant, and otherwise unsuccessful records reject. No adapter or Planner run is claimed.
