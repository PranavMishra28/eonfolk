...
**Browser/visual evidence:** use [Quality Bar's frozen Gate 0 protocol](../../quality/QUALITY_BAR.md#gate-0--product-kill-gate): six people see all six under the complete balanced Williams schedule; fixed facts/choices/cost/fidelity/90-second decision/45-second consequence/15-second view/Replay/60-second reset inside each fixed 225-second slot; exact binary questions and 1-best/6-worst untied rank. Withdrawal/skip is zero and fails the gate. Riverhold must have the unique lowest summed rank, at least 4/6 Yes on each binary, and no alternative may exceed it by two true responses on either binary; keep, all-zero-floor-fail, and comparative-reopen mocks must agree. A separate five-person silent observer manifest uses exact fixture tokens for Mara/three activities/both participants plus change/autonomy. Shell by 2 seconds, CTA by 3.
