...
**Status:** TECHNICAL IMPLEMENTATION CANDIDATE — FINAL REPAIR/CHECKPOINT CI GREEN; CLEAN-COMMIT PERFORMANCE/CONFIRMATION PENDING; HUMAN GATES NOT RUN
...
| DEV-M5-001 / 2026-08-21 | Accept a stable M4 Pro battery profile only at >=50% and macOS `powermode 0`, in addition to AC, for the final local lab. Record both boundaries and fail any power-source change. | Binding overnight acceptance permits measured performance plus justified correction and forbids lowering numerical gates; earlier 15-run battery diagnostic passed by wide margins | No payload/display/frame target changes; finalized harness adds stricter source/dirty/power/pooled oracles | Coordinator; reopen if paired data shows this battery profile is less demanding or materially more variable than AC |
