...
**Status:** COMPLETED AND INTEGRATED THROUGH MEGA PR #7 — retained as regression evidence; human Gate 0/A/B remain not run
...
- [x] Rerun the canonical three-profile 15-run performance/egress battery from exact clean final implementation head `f818d10`; all fifteen repetitions passed with accepted AC power, unchanged source/lockfile, and zero external attempts.
...
- [x] Fresh desktop/laptop/mobile canonical 15-run performance evidence captured from exact clean final implementation head `f818d10`: all declared display/p95/interaction/semantic/power/zero-egress gates passed; output SHA-256 `21e85a3e875ca79d0d6e31b25d4ba19f1c871bb6c22c780cf777ba877cfb689d`. One isolated 50.1 ms laptop-arrival frame is retained as a dropped frame rather than hidden.
...
- 2026-08-21 — Mega PR #7 remote Verify failed closed at the unit stage because the build-identity test injected a missing Git resolver while `resolveBuildSha` still captured ambient `GITHUB_SHA` through its default parameter. The pure resolver now accepts only its explicit argument; Vite passes GitHub/local environment identity at the call site. A local reproduction with `GITHUB_SHA` set plus the complete FAST lattice passed. Because this touches build configuration after `e349c5c`, a fresh clean DEEP run on the repaired commit is required before merge.
- 2026-08-21 — The next remote Verify run passed the hermetic unit test, then Linux software-rendered PlayCanvas made each ordinary phase click take roughly 11–15 seconds; both 30-second fault tests timed out before reaching their injected persistence crash. Retained trace/screenshot evidence showed the buttons visible and the click sequence progressing, not a game-state or selector mismatch. The two persistence-fault journeys now select the supported semantic world before loading, isolating crash recovery from renderer throughput; all sixteen production journeys continue to run against the illustrated world.
- 2026-08-22 — The third remote Verify run proved that fault isolation (2/2 passed) but failed closed when Linux SwiftShader plus retained Playwright tracing stalled ordinary illustrated-world operations: 14/16 production journeys failed after 8.6 minutes, with traces measuring roughly 10 seconds per click and Reality continuing independently. Coordinator trace inspection and a fresh bounded review rejected timeout inflation and fake time. Integrated `8fe81ec` instead gives Linux CI an honest split claim: two semantic fault journeys plus fourteen semantic product journeys, while a blocking trace-free three-viewport smoke proves real PlayCanvas/WebGL2 readiness, exact canvas sizing, bounded pixel ratio, eight-citizen projection/semantic parity, visible activity, desktop keyboard camera operation, zero browser errors, and zero external attempts. The two cadence/spatial-picking journeys remain mandatory in target-Mac PR/DEEP runs. The isolated repair passed 191 FAST tests, 14/14 Linux semantic journeys, 2/2 fault journeys, all 16 local illustrated journeys, and all three renderer-smoke viewports before coordinator integration.
- 2026-08-22 — Before the CI split was integrated, exact clean baseline `a19b31871455d033d12a9a67c3c67b3a68210ac8` passed all 26 DEEP rows and fifteen canonical performance repetitions with unchanged source/lockfile and AC power from 77% to 95%. Pooled p95 frame time was 9.0 ms desktop, 9.1 ms laptop, and 9.2 ms throttled mobile; meaningful-world display was at most 4,428.9 ms; no frame was dropped and all 75 routed performance requests plus netlog oracles had zero external attempts. Output SHA-256 is `80b52d37140d564fbbfa520dac275fdf78534f93c3ab8914f4dab85a982d073b`. This is valid pre-integration evidence, not the final-source release claim; exact final-source DEEP remains required after `8fe81ec`.
- 2026-08-22 — GitHub Actions run [32559456985](https://github.com/PranavMishra28/eonfolk/actions/runs/32559456985) passed required `Verify`, `Formal model`, and full-history `Secret scan` against exact implementation head `f818d1069401a1f8e14d2ca6badec29841afbd81`. Linux recorded nineteen passing PR constituents: two semantic injected-fault journeys, fourteen semantic product journeys, and a trace-free real PlayCanvas/WebGL2 smoke at 1728×1117, 1366×768, and 390×844. Each smoke viewport proved exact canvas containment, pixel ratio 1, eight projected/semantic citizens, visible active tasks, zero browser errors, and zero external attempts; desktop also proved keyboard camera movement. Accepted artifacts `founder-alpha-ui-32559456985` and `verification-manifest-32559456985` use 30-day retention. Output SHA-256 is `4e65374b3b314e7f7e4e22888228c61859171387671082b2759589cdf5f12da2`; artifact-manifest SHA-256 is `a6d87c4ce981d79ca6dfb27d750e6e89b5e3bf45d9cd58f3cb2c8fbfc2cd5c6e`.
- 2026-08-22 — The same exact clean implementation head passed target-Mac `pnpm verify:deep` while plugged into AC power. All 26 rows passed with unchanged source/lockfile: 191 tests in 23 files, 500-run/320-command properties, 8/8 mutations, real IndexedDB, timing, two fault journeys, all sixteen illustrated production journeys, build/budgets/audit, bounded TLC (3,480 generated/350 distinct, depth 10, five invariants), browser cohort, persistence/diagnostics, and fifteen canonical performance runs. Pooled p95 was 9.1 ms desktop, 8.9 ms laptop, and 9.2 ms throttled mobile; meaningful display was at most 445.0 ms, 455.4 ms, and 4,433.4 ms respectively. One isolated 50.1 ms laptop-arrival frame counted as one dropped frame; the declared p95 gate still passed. All 75 routed requests and netlog oracles reported zero external attempts. Output SHA-256 is `21e85a3e875ca79d0d6e31b25d4ba19f1c871bb6c22c780cf777ba877cfb689d`; artifact-manifest SHA-256 is `cc54184974c05e69e9be0c18d98ebd5d07769bac55bf96fcc1b650bf68dedddd`; performance artifact SHA-256 is `d1a8fb491d58bd1dedd2cbfade3af08b7191220f3930afcc50e4c2bb1446278e`.
- 2026-08-22 — Mega PR #7 merged as `405697b`. A bounded continuation on the sole `feat/002-founder-alpha` branch replaces the provisional CSS glyph with one repository-authored SVG mark across the header, favicon, durable loading/failure states, footer, and Story Card; adds root clone/run guidance and `dev`/`prod` entry points; widens renderer-evidence triggers; adds checksum-pinned actionlint; and exposes an intentional portable Linux extended lane while retaining target-Mac-only DEEP authority. Logo-focused unit/build/budget evidence, three required viewport captures, 16/16 target-Mac production journeys, and zero-egress validation passed before the exact clean PR/DEEP reruns. A live remote probe also found and immediately corrected a transient public-visibility drift; no Pages site or fork existed. No Reality, model, deployment, credential, or paid-service scope was added.
...
| Diagnostics/Sentinel | AUTOMATED PASS | redaction, bounds, Sentinel authority, HMR-stable observer, native performance and injected-fault tests; clean source and browser benchmarks passed within the final implementation-head DEEP lattice at `f818d10` |
...
| Founder Alpha browser matrix | EXACT-YES, REMOTE CI, AND CLEAN DEEP PASS | Frozen `90c0ad2` received exact **YES** with zero P0/P1. Exact implementation head `f818d10` passed protected GitHub CI, two fault journeys, all sixteen target-Mac illustrated production journeys, real three-viewport Linux renderer smoke, and all fifteen canonical performance/zero-egress repetitions. |
