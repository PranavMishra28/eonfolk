...
Import is interruption-safe. Acquire `.git/eonfolk-import.lock` by no-follow exclusive create containing checkpoint/source SHA; an existing matching lock triggers recovery, mismatch stops. Verify ledger as a complete newline-terminated prefix. Create destination with `O_NOFOLLOW|O_CREAT|O_EXCL`, mode 0600, and fsync it/directory. Create a same-directory exclusive temp ledger containing the verified prefix plus one line, fsync, atomic-rename/fsync it over the ledger. Stage only destination+ledger and make one commit. Never overwrite/prune evidence. Exact recoverable states for that checkpoint/source are: neither mutation→start; destination-only with identical SHA→complete ledger; destination+valid final ledger line→commit; evidence commit at `HEAD`→run analyzer; analysis commit at `HEAD`→create checkpoint. Any ledger-only state, unexpected diff/index entry, mismatch, non-prefix ledger, or ref movement is P0. Remove the valid lock only after the new checkpoint. A SHA already verified in ledger+destination is a no-op at the current await checkpoint, never a new commit/count; a retry needs a new study/source SHA.
...
- `story-card`: five assignment objects `{studyId:"C01"…"C05",cohortRole:"card-viewer",branchId:<selected literal>}`; `taskTimesMs:{exposureMs:5000|null}`, empty `choices/ratings`, `textResponses:{adviser,choice,followed,directCause,unresolved:string|null}`, and `rubricScores:{allFiveCorrect:boolean|null}`.
...
For imported operator categories create three synthetic segments of `minutes*60` seconds with `ownerId="operator:"+sourceSha256+":"+category`, `localOrdinal:1`, evidenceRef=import destination, and `segmentId=SHA-256(UTF8("EONFOLK-OPERATOR-WORK-v1\n"+sourceSha256+"\n"+category))`. Never also add the minutes elsewhere. Sum every unique segment's seconds—including parallel overlaps—with integer arithmetic; display hours only as `seconds/3600` without rounding for stop decisions. The global ceiling is 216,000 seconds; M0 T01–T03 ceiling is 14,400. Before freezing a human manifest, its operator-minute cap is the floor of remaining global and milestone seconds divided by 60. Before any bounded action, if recorded seconds plus its current high estimate would exceed the applicable ceiling, apply cuts/replan or stop; no segment may begin at the ceiling. Precommit and continuously update this exact bottom-up estimate:
...
License policy applies to every direct/transitive package: automatically permit only SPDX `MIT`, `Apache-2.0`, `BSD-2-Clause`, `BSD-3-Clause`, `ISC`, `0BSD`, `Unlicense`, `CC0-1.0`, `Unicode-3.0`, `BlueOak-1.0.0`, `Python-2.0`, or `Zlib`; for `OR`, record the chosen allowed branch, while every `AND` term must be allowed. Copyleft, source-available, noncommercial, custom/unknown/`SEE LICENSE`, paid, or missing metadata blocks for operator review/removal. Deny lifecycle/install scripts by default; an exception needs line-by-line script review, no credential/network/host mutation, exact evidence and coordinator approval. This planning branch gains no license file; public distribution gets a separate attribution/license audit.
