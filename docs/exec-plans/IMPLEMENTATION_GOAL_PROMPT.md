...
Import is interruption-safe. Let `opKey=sourceSha256`. Resolve absolute `IMPORT_LOCK` with `git rev-parse --path-format=absolute --git-path eonfolk-import.lock`; it is an atomic lock **directory**, never a path under the linked worktree's `.git` file. `LOCK_STATE`, `RECOVERY_A`, and `RECOVERY_B` are exactly `IMPORT_LOCK/state.jcs`, `recovery-a.jcs`, and `recovery-b.jcs`. Before mutation, run the frozen analyzer against the read-only source and precompute exact destination bytes, final ledger bytes, analysis bytes, and full living-plan bytes. Analysis is `RFC8785(object)+"\n"` with exactly `schemaVersion:"eonfolk-analysis-v1"`, `gateId`, `studyCommit`, `sourceSha256`, `analyzerSha256`, `outcome:PASS|FAIL`, and the closed `criterionResults`. Require the existing living plan to end LF; its deterministic target is existing bytes plus exactly `- Evidence import <signedAtUtc>: gate <gateId>; study <studyCommit>; source <sourceSha256>; analyzer <analyzerSha256>; outcome <outcome>.\n`, using the source's canonical UTC sign-off and literal values, with no wall-clock text.

`RECOVERY_A` is `RFC8785(object)+"\n"`, mode 0600, with exactly `version:"eonfolk-import-recovery-a-v1"`, checkpoint/gate/source/study IDs, and for each `destination|ledger|workSegments|analysis|livingPlan` a closed `{path,sha256,base64}` object containing the complete target raw bytes. `workSegments` is the verified existing newline-terminated prefix plus the three operator lines in lexical category order `analysis,facilitation,setup`. Compute A's SHA before mutation. Lock state is `RFC8785(object)+"\n"` with exactly `checkpoint`, `gateId`, `recoveryASha256`, `sourceSha256`, `state:"importing"`, `studyCommit`, and `version:"eonfolk-import-lock-v1"`. Acquire with atomic `mkdir(IMPORT_LOCK,0700)` and parent-directory fsync. Write each lock-internal file through its deterministic exclusive mode-0600 temp `.<basename>.<opKey>.tmp`, fsync, rename, and lock-directory fsync. An existing empty lock directory or exact state/recovery temp-only prefix recreates/finishes state and A from the unchanged source/artifact; exact state without A recreates A and requires its recorded hash. Any other internal entry/bytes stop. After the analysis commit exists, precompute exact resume bytes from that HEAD/outcome and atomically create `RECOVERY_B` with exactly `version:"eonfolk-import-recovery-b-v1"`, `sourceSha256`, `analysisCommit`, and closed resume `{path,sha256,base64}`.

Every target's deterministic same-directory temp is `.<basename>.eonfolk-<opKey>.tmp`, created no-follow/exclusive mode 0600 from its recovery bundle. The only ordinary temp crash states are: exact temp with final absent → fsync/rename/directory-fsync it; exact temp plus exact final → unlink temp/directory-fsync; neither → create temp; any other temp/final bytes or name stops. Verify ledger as a complete newline-terminated prefix before its bundled target. Never overwrite/prune evidence.

The only Git recovery states for the matching lock/bundles, after normalizing those exact temp states, are: (1) no destination/ledger/work-segment mutation: write destination, ledger, then work segments; (2) any exact left prefix of that three-target order: finish the missing suffix; (3) all three exact targets, unstaged or staged: stage exactly those paths and commit evidence/labor; (4) evidence commit at `HEAD`, first parent locked checkpoint: write analysis and living-plan targets; (5) exact analysis and/or living-plan target are the only unstaged/staged changes: finish, stage exactly both, commit analysis; (6) analysis commit at `HEAD`, first parent evidence commit: create B and write resume; (7) exact resume is the only unstaged/staged change: stage only it and commit the resume-only checkpoint; (8) checkpoint commit at `HEAD`, first parent analysis commit and resume names that parent: validate chain, unlink every lock-internal file, `rmdir(IMPORT_LOCK)`, and fsync its parent. A non-prefix target state, ledger-only state, other diff/index/temp entry, mismatch, non-prefix ledger/work-segment file, ref/parent movement, or unlisted state is P0. A SHA already verified in ledger+destination and matching operator segments is a no-op at the current source checkpoint, never a new commit/count; a retry needs a new study/source SHA.
...
- `gate-b`: eight assignment objects `{studyId:"P01"…"P08",cohortRole:"comparison",firstCondition:real|control}` matching the frozen P-ID shuffle, exactly four each. `taskTimesMs` has `real|control` × `AdviceConfirmMs` (`0..300000|null`) and `SecondActionMs` (`0..60000|null`); `choices` has each condition's `Advice` (`verify-private|accuse-now|abstain|null`) and `SecondActionInitiated` (`boolean|null`); `ratings` has each condition's `Contingency|Continue` (`integer 1..7|null`); `textResponses` has each condition's `VisibleReason|Ownership|LaterChange|SaveLocation|SecondActionWhy` (`string|null`); `rubricScores` has each condition's `FourPromptsCorrect|PersonCenteredReason` (`boolean|null`).
- `story-card`: five assignment objects `{studyId:"C01"…"C05",cohortRole:"card-viewer",branchId:<selected literal>}`; `taskTimesMs:{exposureMs:integer 5000..6000|null}`, empty `choices/ratings`, `textResponses:{adviser,choice,followed,directCause,unresolved:string|null}`, and `rubricScores:{allFiveCorrect:boolean|null}`.

`operatorSignoff` has `additionalProperties:false` and exactly `attests:true`, `role:"authorized-human-operator"`, `signedAtUtc` matching canonical UTC `^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z$` and a real calendar instant, and statement `I attest eligibility, affirmative agreement, faithful administration, raw response entry, withdrawals, focused-minute categories excluding participant response and scheduling wait, and no replacement or PII/recording.` Every count/score is recomputed from records; the operator supplies no pass boolean.
...
```json
{
  ...
  "gate-b": {
    ...
    "questions": [
      ...
      {"id":"second-action-why","prompt":"Why take that action?","responseType":"string"},
...
```json
{
  "gate-0":["records-complete","h-unique-rank","h-desirable-3of5","h-continue-3of5","no-desirable-lead-20pp","no-continue-lead-20pp","observer-activity-interaction-3of5","observer-autonomy-4of5"],
...
`records-complete` means the exact assignments/cardinality/order/schema/attestations pass. For Gate 0 it is false when **any** of its ten records has `abandoned:true`, any product record lacks a rank permutation, or any product variant has null Advice, Desirable, Continue, Prediction, or Objection after its deadline; that last condition is the exact skipped-variant predicate, while null Replay is exempt. It is false for Gate B on withdrawal or any missing condition rating. For Gate A and Story Card, an enrolled abandonment stays as a null/zero record in the denominator and does not by itself make structural completeness false. `h-unique-rank` means H has the sole lowest within-person rank sum. The two H floors count literal true among five product records. With the common denominator five, each lead check uses integer counts only: every competitor's true count must be `<=` H's true count; one additional true would be the forbidden 20-point lead. The Gate 0/A activity criterion counts a record only when Mara, three activities, both actors, and the state change rubric booleans are all true; autonomy counts only `followMaraFindMs<=10000`, `saysCannotCommand:true`, and `selfDirectionReason:true`. Gate B advice counts non-null real advice confirmed by 300,000 ms; four-prompts counts `realFourPromptsCorrect:true`; second action counts both `realSecondActionInitiated:true` and `realPersonCenteredReason:true`. Means use all eight non-null integer ratings per condition and compare exact integer sums (`realSum>controlSum`), never rounded means. Story Card counts `allFiveCorrect:true`. Missing/null never contributes a pass; Gate 0 Replay is diagnostic only.
...
4. Sum within-person ranks; lowest total wins. Reopen unless `H` has the unique lowest total **and at least 3/5 Yes on both Desirable and Continue**. Also reopen if any other ID leads `H` by at least 20 percentage points (one of five participants) on either of those two binaries. Do not proceed on qualitative preference. The committed analyzer includes one keep, one all-zero absolute-floor failure, and one comparative reopen mock table; two reviewers must obtain the same result.
