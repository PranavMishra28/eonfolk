...
An existing path/branch normally resumes only from a clean current `HEAD` whose sole diff from its first parent is `docs/exec-plans/evidence/001/resume.json`. That file is RFC 8785 JSON with `additionalProperties:false` and exactly: `schemaVersion:"eonfolk-resume-v1"`; `planBase:PLAN_BASE`; `worktreePath:"/Users/pranav/Documents/ChatGPT/.eonfolk-worktrees/implementation"`; `branch:"codex/eonfolk-001-foundation"`; lowercase 40-hex `expectedImplementationHead` equal to the checkpoint commit's first parent; `phase` in `await-gate-0|work-after-gate-0|await-gate-a|work-after-gate-a|await-gate-b|await-story-card|work-after-gate-b|complete`; and `nextEvidence`. `nextEvidence` is respectively `gate-0-human.json`, null, `gate-a-human.json`, null, `gate-b-human.json`, `story-card-human.json`, null, null.

Same-phase checkpoint renewal is legal. A gate import advances only after its frozen analyzer returns PASS: `await-gate-0→work-after-gate-0→await-gate-a→work-after-gate-a→await-gate-b→await-story-card→work-after-gate-b→complete`. A FAIL/void result commits the failure and returns to the same `await-*` phase only after an allowed in-scope fix plus a new artifact/study commit; every prior cohort remains. Gate 0 product failure has no autonomous retry and stops for operator product authority; its permitted visual simplification may create one new full Gate 0 cohort. Gate A/B/card may retry only the failed gate after logged in-scope fixes and new independent confirmation. No passed gate moves backward. Before every voluntary pause, commit integrated work, then create this manifest-only checkpoint with the correct phase and stop clean. Any other mismatch/collision/dirty state stops for operator direction except the exact interrupted-import recovery states below.
...
Import is interruption-safe. Resolve `IMPORT_LOCK` once with `git rev-parse --path-format=absolute --git-path eonfolk-import.lock`; require that absolute path, never `.git/eonfolk-import.lock`. Its exact UTF-8/RFC 8785 bytes are `{"checkpoint":"<40hex>","gateId":"<gateId>","sourceSha256":"<64hex>","state":"importing","studyCommit":"<40hex>","version":"eonfolk-import-lock-v1"}\n`, with only the bracketed values substituted. Create it with `O_NOFOLLOW|O_CREAT|O_EXCL`, mode 0600, then fsync the file and parent directory. An existing byte-identical lock triggers recovery; any mismatch stops. Verify ledger as a complete newline-terminated prefix. Create destination with `O_NOFOLLOW|O_CREAT|O_EXCL`, mode 0600, and fsync it/directory. Create a same-directory exclusive temp ledger containing the verified prefix plus one line, fsync, atomic-rename and fsync it over the ledger. Stage only destination+ledger and make one commit. Never overwrite/prune evidence.

The only recoverable states for the matching lock/checkpoint/source, in order, are: (1) no destination/ledger mutation: begin; (2) identical destination only: complete the ledger; (3) identical destination plus the one valid new final ledger line, whether unstaged or staged: stage exactly those two paths and create the evidence commit; (4) evidence commit at `HEAD`, with first parent equal to the locked checkpoint: run the analyzer; (5) the deterministic analysis path and/or the living-plan path are the only unstaged/staged changes: recompute their exact expected bytes, require every present byte to match, atomically finish any missing file, stage exactly those two paths, and create the analysis commit; (6) analysis commit at `HEAD`, with first parent equal to the evidence commit: create the resume file; (7) the exact deterministic `resume.json` is the only unstaged/staged change: require byte equality, stage only it, and create the checkpoint commit; (8) checkpoint commit at `HEAD`, with first parent equal to the analysis commit and its resume naming that parent: validate the full chain, unlink the lock, and fsync the lock directory. Analysis, full living-plan replacement, and resume bytes are each precomputed, written to an exclusive same-directory temp, fsynced, atomically renamed, and directory-fsynced; recovery accepts their exact partial presence. Any ledger-only state, other diff/index entry, byte mismatch, non-prefix ledger, ref movement, parent mismatch, or unlisted state is P0. Remove the valid lock only in state 8. A SHA already verified in ledger+destination is a no-op at the current await checkpoint, never a new commit/count; a retry needs a new study/source SHA.

After the evidence commit, run the study artifact's analyzer. Commit `docs/exec-plans/evidence/001/human/analysis/<gateId>/<studyCommit>-<sourceSha256>.json` plus the living-plan entry. Analysis JSON has exactly `schemaVersion:"eonfolk-analysis-v1"`, `gateId`, `studyCommit`, `sourceSha256`, `analyzerSha256`, `outcome:PASS|FAIL`, and closed `criterionResults` booleans named by the prompt's gate criteria. Then create a manifest-only checkpoint: advance only on PASS; on FAIL use the same await phase under the retry rules. Import/labor counts remain once per source SHA.
...
The following JSON literals close the instrument contract. The consent string is byte-identical in all four `script` arrays. `{variantId}` is replaced only by the assigned member of `H|FAM|TRI|FAC|ECH|DIR`; `{conditionId}` only by `real|control`; substitution is one NFC value with no braces and no other script edits.
...
```json
{
  "gate-0": {
    "script": [
      ...
      "Product participants: You will review six unbranded versions in your assigned order. Begin {variantId}. Choose one available action within 90 seconds without outside explanation.",
      ...
      "Visual observers: Watch silently for 60 seconds. Do not open or use a raw event log, and do not ask for narration.",
      "At the prompt, answer only from what you observed; then activate Follow Mara and answer the autonomy question."
    ],
    "questions": [
  ...
  "gate-a": {
    "script": [
      "This is a voluntary unpaid prototype study. We record only an anonymous study ID, task timing, choices, ratings, comments, and written screen-observation notes; no name or contact details, audio, or video. You may skip or stop at any time without consequence. Do you agree to participate and to this anonymous data collection?",
      "Watch silently for 60 seconds. Do not open or use a raw event log, and do not ask for narration.",
      "At the prompt, answer only from what you observed; then activate Follow Mara and answer the autonomy question."
    ],
    "questions": [
      {"id":"point-mara","prompt":"Point to Mara.","responseType":"string"},
  ...
  "gate-b": {
    "script": [
      "This is a voluntary unpaid prototype study. We record only an anonymous study ID, task timing, choices, ratings, comments, and written screen-observation notes; no name or contact details, audio, or video. You may skip or stop at any time without consequence. Do you agree to participate and to this anonymous data collection?",
      "You will play two unbranded conditions in your assigned order. Begin {conditionId}. Use only the visible interface and decide without outside explanation.",
      "After advice, use Leave Riverhold, Advance Riverhold, and Return; then decide whether to take the available second action.",
      "Answer the five condition questions in order before beginning the other condition."
    ],
    "questions": [
...
For imported operator categories create three synthetic segments of `minutes*60` seconds with `ownerId="operator:"+sourceSha256+":"+category`, `localOrdinal:1`, `taskId` from the gate map below, evidenceRef=import destination, and `segmentId=SHA-256(UTF8("EONFOLK-OPERATOR-WORK-v1\n"+sourceSha256+"\n"+category))`. Gate 0's setup/facilitation/analysis all map to T02; Gate A's to T12; Gate B's and Story Card's to T15. Never also add the minutes elsewhere. Sum every unique segment's seconds—including parallel overlaps—with integer arithmetic; display hours only as `seconds/3600` without rounding for stop decisions. The global ceiling is 216,000 seconds. Milestone/task ceilings are exact: M0/T01–T03 14,400; M1/T04–T06 28,800; M2/T07–T09 28,800; M3/T10–T12 43,200; M4/T13–T15 43,200; M5/T16–T18 28,800 seconds. A segment counts against the milestone containing its `taskId`; no milestone borrows another's unused ceiling. Before freezing a human manifest, its operator-minute cap is the floor of the lesser remaining global and mapped-milestone seconds divided by 60. Before any bounded action, if recorded seconds plus its current high estimate would exceed the applicable ceiling, apply cuts/replan or stop; no segment may begin at the ceiling. Precommit and continuously update this exact bottom-up estimate:
...
All study timers use foreground-page `performance.now()` elapsed time. Let `rawElapsedMs=performance.now()-origin` as the browser's nonnegative double. Thresholds compare that raw double; persisted elapsed values are `Math.ceil(Math.max(0,rawElapsedMs))` integers. A Gate 0 product variant starts when its three choices and first world state are painted, enabled, focusable, and its observer asserts the correct variant ID. Its decision endpoint is the first confirmed choice with raw elapsed `<=90000`, or timeout at the first frame with raw elapsed `>=90000`; set `confirmationTime` on confirmation and reveal consequence only on the first frame with `performance.now()-confirmationTime >=45000`. Gate 0 visual and Gate A start at the first animation frame where Mara plus all eight projections/current activities/interaction cue and equivalent semantic rows are painted and Follow Mara is enabled/focusable. Start Follow Mara find timing and the observation clock on that same frame; activation with raw elapsed `<=10000` passes. Administer the observer prompt on the first frame with raw elapsed `>=60000`; require raw elapsed `<=61000` and persist its converted value, otherwise the attempt fails for timer delivery. Story Card exposure analogously prompts on the first frame `>=5000`, must be `<=6000`, and persists the converted elapsed value. Any focus/visibility loss, navigation, reload, clock reset, fixture mismatch, operator pause, or timer-delivery overrun before an endpoint invalidates that participant's attempt as failure with no restart/replacement; study timers never pause. Gate B uses its separately defined equivalent frames below and the same raw-comparison/conversion rule.
...
For **each** condition, start the five-minute advice clock at the first painted frame where the frozen visible facts, Mara values/relationships, all three advice choices, equivalent semantic rows, and Follow Mara are present and the choices/Follow Mara are enabled/focusable; assert snapshot/condition IDs on that frame. After advice confirmation (or timeout), run the frozen leave/advance/return sequence. Start the 60-second second-action clock at the first painted frame where the changed return world, branch result, equivalent semantic facts, and its branch-legal action are visible and that action is enabled/focusable. The Gate 0/A focus/visibility/navigation/reload/clock/fixture/pause invalidation, timeout, no-restart and no-replacement rules apply independently to both Gate B conditions. After that action or timeout, and before showing the other condition, ask the four frozen comprehension prompts in the listed order and then ask exactly: `Contingency: How much did Mara's choice and the outcome depend on the visible facts, her values/relationships, and your advice?` from 1 `not at all; it seemed fixed or unrelated` to 7 `strongly; changing those inputs could change it`; and `Continue: How much do you want to take the next Riverhold action now?` from 1 `not at all` to 7 `very strongly`. Missing either rating in either condition or withdrawal automatically fails Gate B; never impute or replace.
...
Timing oracle: derive raw `timingSeed=SHA-256(UTF8("EONFOLK-VIS-TIMING-v1\n"+PLAN_BASE+"\n"+implementationCommit+"\n"+snapshotHash))`, seed the defined xoshiro stream with `(system="security",entityId="riverhold-visibility-v1",purpose="timing-order")`, and run 20 unrecorded warmups per class. Then run exactly 200 cycles; each cycle rejection-sampled Fisher–Yates shuffles `[hidden,missing,revoked]` and issues one request per class, giving 200 retained samples/class. Measure foreground `performance.now()` from application receipt to presented response. Retain every sample; each is ≥50 ms, and pairwise class median and nearest-rank p95 (`sorted[ceil(.95*n)-1]`) differences are each ≤5 ms. One committed implementation/snapshot/profile gets one committed run; failure requires a code change/new commit and a wholly new run, never a favorable same-build rerun. Retain raw order/samples/analyzer/seed plus failed artifacts 14 days and accepted 30. Code-path inspection plus this oracle defines timing equality.
