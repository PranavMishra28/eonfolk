...
Same-phase resume-only checkpoint renewal is legal. A gate import advances only after its frozen analyzer returns PASS: `await-gate-0→work-after-gate-0→await-gate-a→work-after-gate-a→await-gate-b→await-story-card→work-after-gate-b→complete`. Before creating the Gate B source checkpoint, freeze both Gate B S and Story Card S; the Gate B resume names Gate B S, and its PASS resume deterministically names the already-committed Story Card S. A FAIL analysis immediately creates the same-phase `remediation` checkpoint and may not read any inbox source. It remains there until an allowed in-scope fix, new artifact commit A, new study commit S, and independent confirmation are committed; then create a same-phase `await-source` checkpoint naming that S before reading a new source. Every prior cohort remains. Gate 0 product failure has no autonomous retry and stops for operator product authority; its permitted visual simplification may create one new full Gate 0 cohort. Gate A/B/card may retry only the failed gate under that remediation sequence. No passed gate moves backward. Before every voluntary pause, commit integrated work, then create this resume-only checkpoint with the correct phase/state and stop clean. Any other mismatch/collision/dirty state stops for operator direction except the exact interrupted-import recovery states below.
...
Before a cohort, set `STUDY_DIR="docs/exec-plans/evidence/001/studies/"+gateId` and commit build/instrument/analyzer as artifact commit `A`. Put the canonical RFC 8785 snapshot at `STUDY_DIR/snapshot.jcs.json`, analyzer at `STUDY_DIR/analyze.mjs`, and schema validator at `STUDY_DIR/validate.mjs`; manifest path fields store these expanded repository-relative paths, and hashes are SHA-256 of raw file bytes. Create `STUDY_DIR/manifest.json` with `additionalProperties:false` and exactly: `schemaVersion:"eonfolk-study-manifest-v1"`; literal `gateId`; `artifactCommit:A`; `snapshotPath`, `snapshotHash`; 64-hex `seed`; `assignments`; `script`; `questions`; `anchors`; `timersMs`; `operatorFocusedMinutesCap`; `scoringProgramPath`, `scoringProgramHash`; `validatorProgramPath`, `validatorProgramHash`.

`assignments` is an array ordered lexically by unique `studyId`, with exact cardinality below; every assignment object rejects extra fields. `script` is an ordered array of NFC strings byte-equal to this prompt's consent/task administration statements. `questions` is an ordered array of closed `{id:NFC string,prompt:NFC string,responseType:"boolean"|"integer"|"string"}` objects byte-equal to the literal questions below. Question IDs, in administration order, are Gate 0 product `desirable,continue,prediction,objection,replay,final-rank`; Gate 0/A observer `point-mara,activities,interaction-change,autonomy`; Gate B per condition `visible-reason,ownership,later-change,save-location,second-action-why,contingency,continue`; Story Card `adviser,choice,followed,direct-cause,unresolved`. `anchors` is an ordered array of closed `{questionId,min:integer,max:integer,minLabel:NFC string,maxLabel:NFC string}` objects; it contains `final-rank:1..6 most..least`, Gate B `contingency:1..7` and `continue:1..7` with the literal labels below; binary/text questions omit anchors. `timersMs` is exactly: Gate 0 `{variantDecision:90000,consequenceDelay:45000,consequenceView:15000,replayResponse:15000,neutralReset:60000,totalVariantSlot:225000,followFind:10000,observerPrompt:60000}`; Gate A `{followFind:10000,observerPrompt:60000}`; Gate B `{advice:300000,secondAction:60000}`; Story Card `{exposure:5000}`. The gate operator maxima are Gate 0 180, Gate A 240, Gate B 300, and Story Card 60 minutes. `operatorFocusedMinutesCap` equals the lesser gate maximum and `floor(min(globalEffectiveRemainingSeconds,mappedEffectiveRemainingSeconds)/60)` at freeze; its committed value reserves that capacity and the human file's three categories must sum within it.
...
Each participant record has `additionalProperties:false` and exactly `studyId`, `cohortRole`, `eligible:boolean`, `affirmativeAgreement:boolean`, `assignment`, `taskTimesMs`, `choices`, `ratings`, `textResponses`, `rubricScores`, `observationNotes:string|null`, and `abandoned:boolean`. `record.assignment` deep-equals its corresponding manifest assignment, `record.studyId==record.assignment.studyId`, and `record.cohortRole==record.assignment.cohortRole`; all maps reject extra keys. Each non-null text answer/note is ≤512 UTF-8 bytes and the whole raw evidence file is ≤204,800 bytes; no PII/audio/video. Every required key remains after abandonment; unavailable values, including observation notes, are null and score failure/zero. Only enrolled records exist, so `eligible` and `affirmativeAgreement` are true. Gate-specific closed shapes are:

- `gate-0`: eleven assignment shapes are `{studyId:"P01",cohortRole:"product",rowId:"R0"}` through `{studyId:"P06",cohortRole:"product",rowId:"R5"}` and `{studyId:"V01",cohortRole:"visual-observer",fixtureId:"gate0-visual-v1"}` through V05. Product maps use opaque presentation positions `V1`…`V6`, never treatment IDs: `taskTimesMs` has each `<Vn>MeaningfulActionMs` (`integer 0..90000|null`); `choices` has each `<Vn>Advice` (`verify-private|accuse-now|abstain|null`) and `<Vn>Desirable|Continue|Replay` (`boolean|null`); `ratings` has each `<Vn>Rank` (`integer 1..6|null`, a permutation when not abandoned); `textResponses` has each `<Vn>Prediction|Objection` (`string|null`); `rubricScores` is `{}`. The analyzer validates all raw position records before using hidden `rowId` to map positions to treatment IDs. Visual records use `taskTimesMs:{followMaraFindMs:integer 0..10000|null,observationPromptMs:integer 60000..61000|null}`, empty `choices/ratings`, `textResponses:{mara,activities,interaction,autonomy:string|null}`, and `rubricScores:{}`; these four strings are opaque UI-selection tokens, not operator judgments.
- `gate-a`: five assignment objects `{studyId:"A01"…"A05",cohortRole:"visual-observer",fixtureId:"gate-a-life-v1"}`; record maps are the Gate 0 visual shape.
- `gate-b`: eight assignment objects `{studyId:"P01"…"P08",cohortRole:"comparison",firstCondition:real|control}` matching the frozen P-ID shuffle, exactly four each. Raw record maps use only opaque presentation ordinals `first|second`: `taskTimesMs` has each ordinal's `AdviceConfirmMs` (`0..300000|null`) and `SecondActionMs` (`0..60000|null`); `choices` has each ordinal's `Advice` (`verify-private|accuse-now|abstain|null`) and `SecondActionInitiated` (`boolean|null`); `ratings` has each ordinal's `Contingency|Continue` (`integer 1..7|null`); `textResponses` has each ordinal's `VisibleReason|Ownership|LaterChange|SaveLocation|SecondActionWhy` (`string|null`); `rubricScores` is `{}`. The analyzer validates and scores both ordinal responses against the hidden receipt-derived answer keys before reading `firstCondition`, then maps the already-scored ordinal results to real/control.
- `story-card`: five assignment objects `{studyId:"C01"…"C05",cohortRole:"card-viewer",branchId:<selected literal>}`; `taskTimesMs:{exposureMs:integer 5000..6000|null}`, empty `choices/ratings`, `textResponses:{adviser,choice,followed,directCause,unresolved:string|null}`, and `rubricScores:{}`.

`operatorSignoff` has `additionalProperties:false` and exactly `attests:true`, `role:"authorized-human-operator"`, `signedAtUtc` matching canonical UTC `^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z$` and a real calendar instant, and statement `I attest eligibility, affirmative agreement, faithful administration, raw response entry, withdrawals, focused-minute categories excluding participant response and scheduling wait, and no replacement, scoring, PII, or recording.` The operator file contains raw instrument responses only: every `rubricScores` object is empty, and the operator supplies no derived boolean, condition classification, pass result, or interpretation.
...
`records-complete` means the exact assignments/cardinality/order/schema/attestations pass. For Gate 0 it is false when **any** of its eleven records has `abandoned:true`, any product record lacks a rank permutation, or any product presentation has null Advice, Desirable, Continue, Prediction, or Objection after its deadline; that last condition is the exact skipped-presentation predicate, while null Replay is exempt. It is false for Gate B on withdrawal or any missing condition rating. For Gate A and Story Card, an enrolled abandonment stays as a null/zero record in the denominator and does not by itself make structural completeness false. `h-unique-rank` means H has the sole lowest mapped within-person rank sum. The two H floors count literal true among six product records. With common denominator six, a competitor leads H by at least 20 percentage points only when its integer true count is at least `Hcount+2`; therefore each lead criterion is true exactly when every competitor count is `<=Hcount+1`. The Gate 0/A activity criterion counts a record only when its four exact raw tokens match Mara, sorted three activities, both actors/change, and the hidden answer key; autonomy additionally requires `followMaraFindMs<=10000` and an exact `cannot-command|<accepted-visible-reason-token>` match. Gate B advice counts non-null advice in the ordinal mapped to real and confirmed by 300,000 ms; four-prompts counts records whose real ordinal's four response tokens all exactly match; second action counts a true real-ordinal initiation plus one of the four person-centered reason tokens. Means use all eight non-null integer ratings per mapped condition and compare exact integer sums (`realSum>controlSum`), never rounded means. Story Card counts records whose five exact tokens all match. Missing/null never contributes a pass; Gate 0 Replay is diagnostic only.
